...
DynamicSynapse::DynamicSynapse(const SynapseParams& params, int seed)
{
  ...
  mt19937 mt(seed);
  normal_distribution<double> randn_u0(params.u0_mean, params.u0_sd);
  normal_distribution<double> randn_tau_d(params.tau_d_mean, params.tau_d_sd);
  normal_distribution<double> randn_tau_f(params.tau_f_mean, params.tau_f_sd);

  while (true)
  {
    double cnd = randn_u0(mt);
...
void DynamicSynapse::calc_grad(double dt)
{
  double spike = pre_neuron->get_spike();
