...
PopSANEncoder::PopSANEncoder(
    ...
    : obs_dim{ obs_dim }, pop_dim{ pop_dim }, spike_ts{ spike_ts }, std{ std }, \
    v_reset{ v_reset }, v_th{ v_th }
{
...
LIFEncoder::LIFEncoder(
    ...
    : obs_dim{ obs_dim }, pop_dim{ pop_dim }, std{ std }, \
    v_rest{ v_rest }, v_reset{ v_reset }, v_th{ v_th }, tau_m{ tau_m }, amp{ amp }
{
    encoder_neuron_dim = obs_dim * pop_dim;
    mean = vector<float>(pop_dim);
    float delta_mean = (mean_range.second - mean_range.first) / (float)(pop_dim - 1);
